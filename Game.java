...
// NOTICE: code has been modified so that orders always issue half the number
// of available ships in the source planet.
...
public class Game implements Cloneable {
	...
	public String PovRepresentation(int pov) {
		...
		for (Planet p : planets) {
			...
			s.append("P " + p.X() + " " + p.Y() + " " + PovSwitch(pov, p.Owner()) + " " + p.NumShips() + " " + p.GrowthRate() + "\n");

		}
		for (Fleet f : fleets) {
			s.append("F " + PovSwitch(pov, f.Owner()) + " " + f.NumShips() + " " + f.SourcePlanet() + " " + f.DestinationPlanet() + " "
	...
	// MODIFIED:
	// Removed argument numShips; we always send half of the ships in
	// sourcePlanet
	public int IssueOrder(int playerID, int sourcePlanet, int destinationPlanet) {
		Planet source = planets.get(sourcePlanet);
		int numShips = source.NumShips() / 2;
		if (source.Owner() != playerID || numShips > source.NumShips() || numShips <= 0) {

			if (source.Owner() != playerID) {
				WriteLogMessage("ERROR: cannot send ships from planet you do not own.");
			}
		...
		Fleet f = new Fleet(source.Owner(), numShips, sourcePlanet, destinationPlanet, distance, distance);
	...
	// NOTICE: modified to parse only two integers (source and destination)
	public int IssueOrder(int playerID, String order) {
		...
		return IssueOrder(playerID, sourcePlanet, destinationPlanet);
	}
	...
	private Point getPlanetPos(Planet p, double top, double left, double right, double bottom, int width, int height) {
	...
	void Render(int width, // Desired image width
		...
		for (Planet p : planets) {
			...
			int r = (int) Math.min(size / (right - left) * width, size / (bottom - top) * height);
			...
			java.awt.geom.Rectangle2D bounds = fm.getStringBounds(Integer.toString(p.NumShips()), g);
			x -= bounds.getWidth() / 2;
			y += fm.getAscent() / 2;
		...
		for (Fleet f : fleets) {
			...
			java.awt.geom.Rectangle2D textBounds = fm.getStringBounds(Integer.toString(f.NumShips()), g);
			g.setColor(GetColor(f.Owner(), colors).darker());
			g.drawString(Integer.toString(f.NumShips()), (int) (x - textBounds.getWidth() / 2), (int) (y + textBounds.getHeight() / 2));
		}
	}
